...
module Grid3D = struct
  ...
  let empty_board =
    ...
    let horizontal_3d_group c b =
      ...
      match c.cell with
        | (x,0,z) when (x=z) -> List.filter (fun a -> (a.cell |> fst')=(a.cell |> thd)) grid_space (*cell list of length 2*)
        | _,0,_ -> List.filter (*cell list of length 2*)
                  (fun a -> a.cell <> (0,0,1) && a.cell <> (1,0,0) && a.cell <> (1,0,2) && a.cell <> (2,1,0)) grid_space
        | x,1,z when (x=z) -> List.filter (fun a -> (a.cell |> fst')=(a.cell |> thd)) grid_space
        | _,1,_ -> List.filter
                  (fun a -> a.cell <> (2,1,1) && a.cell <> (1,1,2) && a.cell <> (0,1,1) && a.cell <> (1,1,0)) grid_space
        | x,2,z when (x=z) -> List.filter (fun a -> (a.cell |> fst')=(a.cell |> thd)) grid_space
        | _ -> List.filter
                  (fun a -> a.cell <> (2,2,1) && a.cell <> (1,2,2) && a.cell <> (0,2,1) && a.cell <> (1,2,0)) grid_space

    let vertical_3d_groups c b =
      let grid_space = board_list_of_cells b in
       match c.cell with
        | x,y,0 when (x=y) -> List.filter (fun a -> (a.cell |> fst')=(a.cell |> snd')) grid_space
        | _,_,0 -> List.filter
                  (fun a -> a.cell <> (0,1,0) && a.cell <> (1,0,0) && a.cell <> (2,1,0) && a.cell <> (1,2,0)) grid_space
        | x,y,1 when (x=y) -> List.filter (fun a -> (a.cell |> fst')=(a.cell |> snd')) grid_space
        | _,_,1 -> List.filter
                      (fun a -> a.cell <> (1,0,1) && a.cell <> (0,1,1) && a.cell <> (1,2,1) && a.cell <> (2,1,1)) grid_space
        | x,y,2 when (x=y) -> List.filter (fun a -> (a.cell |> fst')=(a.cell |> snd')) grid_space
        | _ -> List.filter
                  (fun a -> a.cell <> (1,0,2) && a.cell <> (1,2,2) && a.cell <> (2,1,2) && a.cell <> (0,1,2)) grid_space

    let diag_check c b =
      let grid_space = board_list_of_cells b in
      let diag_h = horizontal_3d_group c grid_space in
      let diag_v = vertical_3d_groups c grid_space in
      let verdict_h = (List.for_all (fun x -> x.player = c.player) diag_h)
      (*checks whether 2 of 3-in-row instance is true*)
    ...
    let win_evaluation c b =
      ...
      match get_plane c.cell with (*TopPlane*)
      | 0 -> case_1 || cases_3d
      | 1 -> case_2 || cases_3d
      | 2 -> case_3 || cases_3d
